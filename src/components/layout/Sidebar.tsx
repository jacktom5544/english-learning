...
export default function Sidebar() {
  ...
  const pathname = usePathname();
  const { points, pointsUsedThisMonth, isLoading } = useUserPoints();
  ...
  return (
    <>
            ...
            <div className="flex flex-col">
              ...
              <div className="text-gray-300 text-xs mb-3">
                {isLoading || points === null ? (
                  ...
                  <>
                    <div>今月の残りのポイント：{points}</div>
                    <div>今月の消費ポイント：{pointsUsedThisMonth}</div>
                  </>
                )}
              </div>
              <button
      ...
      {/* Mobile Sidebar */}
      <div className={`lg:hidden fixed inset-0 z-30 bg-opacity-50 transition-opacity duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
        <div className={`fixed inset-y-0 left-0 flex flex-col w-64 bg-gray-900 overflow-y-auto transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
